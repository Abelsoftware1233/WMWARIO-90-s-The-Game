...
// Responsive canvas — fills available space between HUD and mobile controls
function resizeCanvas() {
  const wrapper = document.getElementById('game-wrapper');
  const hud = document.getElementById('hud');
  const controls = document.getElementById('mobile-controls');

  const totalH = wrapper.clientHeight || window.innerHeight;
  const hudH = hud ? hud.offsetHeight : 0;
  const ctrlH = controls ? controls.offsetHeight : 0;
  const availH = totalH - hudH - ctrlH;

  // On desktop cap at 800px wide; on mobile fill width
  const maxW = Math.min(wrapper.clientWidth, 800);
  // Prefer 400×240 aspect ratio (5:3), but clamp to available height
  let w = maxW;
  let h = Math.round(w * (240 / 400));
  if (h > availH && availH > 0) {
    h = availH;
    w = Math.round(h * (400 / 240));
    if (w > maxW) w = maxW;
  }
...
// Re-size when orientation changes (mobile)
...
// Mobile buttons — multi-touch aware
...
if (mc) mc.addEventListener('contextmenu', e => e.preventDefault());
...
class Mario extends Entity {
  ...
  update(level) {
    ...
    // Variable jump height — capped so Mario can't fly indefinitely upward
    if (jumpBtn && this.vy < -4 && this.vy > -13 && !this.onGround) {
      this.vy -= 0.3;
    }
    ...
    if (level.width && this.x + this.w > level.width) { this.x = level.width - this.w; this.vx = 0; }
  ...
  collideTiles(level) {
    ...
    for (const tile of level.tiles) {
      ...
      const tx = tile.gx * t;  // world-space
      const ty = tile.gy * t;
  ...
  draw() {
    ...
    const x = Math.round(this.x - game.cameraX);
...
class Goomba extends Entity {
  ...
  collideTilesSimple(level) {
    ...
    if (this.y > canvas.height + TILE() * 2) this.alive = false;
  }
  draw() {
    const t = TILE();
    const x = Math.round(this.x - game.cameraX); const y = Math.round(this.y);
    if (this.squished) {
...
class PiranhaPlant extends Entity {
  ...
  draw() {
    drawPixelSprite(PIRANHA, Math.round(this.x - game.cameraX), Math.round(this.y));
  }
}
...
class BulletBill extends Entity {
  ...
  update() {
    this.x += this.vx;
    if (this.x < -TILE() * 2) this.alive = false;
  }
  draw() {
    drawPixelSprite(BULLETBILL, Math.round(this.x - game.cameraX), Math.round(this.y));
  }
}
...
class Spiny extends Entity {
  ...
  draw() {
    drawPixelSprite(SPINY, Math.round(this.x - game.cameraX), Math.round(this.y), this.frame===1);
  }
}
...
class Boo extends Entity {
  ...
  draw() {
    ...
    drawPixelSprite(BOO, Math.round(this.x - game.cameraX), Math.round(this.y), this.frame===1);
    ctx.globalAlpha = 1;
  }
}
...
class Lakitu extends Entity {
  ...
  draw() {
    ...
    drawPixelSprite(LAKITU_CLOUD, sx, Math.round(this.y + t*0.6));
    // Lakitu body (simple)
    drawRect(sx + t*0.3, this.y, t*0.8, t*0.6, C.marioW);
    drawRect(sx + t*0.35, this.y+2, t*0.7, t*0.4, C.marioR);
  }
}
...
class BowserBoss extends Entity {
  ...
  update(level, mario, fireballs) {
    ...
    // Bounds — world-space
    const minX = level.width * 0.5;
    const maxX = level.width - this.w - TILE();
    ...
    // Fire — shoot toward Mario
    this.fireTimer++;
    if (this.fireTimer > 60) {
      this.fireTimer = 0;
      const shootLeft = this.x > mario.x;
      fireballs.push({ x: this.x + (shootLeft ? 0 : this.w), y: this.cy, vx: shootLeft ? -4 : 4, vy: -2, alive: true });
    }
    this.tickFrame(10);
  }
  draw() {
    drawPixelSprite(BOWSER, Math.round(this.x - game.cameraX), Math.round(this.y), this.vx > 0);
  }
  hit() {
    this.hp--;
    return this.hp <= 0;
...
function hitBlockFromBelow(tile, level) {
  if (tile.type === 'qblock' && !tile.hit) {
    ...
    // Spawn content in world-space
    const t = TILE();
    const wx = tile.gx * t;  // world-space
    const wy = tile.gy * t;
    ...
    if (roll < 0.5) {
      ...
      addScore(200, wx - game.cameraX, wy);
      game.coins++;
      playSound('coin');
    } else if (roll < 0.75) {
    ...
    addScore(100, wx - game.cameraX, wy);
...
function buildLevel(worldNum, levelNum) {
  ...
  if (worldNum === 1 && levelNum === 1) {
    ...
    // Enemies — world-space positions
    enemies.push(new Goomba(25*t, (groundRow-1)*t));
    enemies.push(new Goomba(40*t, (groundRow-1)*t));
    enemies.push(new KoopaTroopa(55*t, (groundRow-1)*t));
    enemies.push(new Goomba(70*t, (groundRow-1)*t));
    enemies.push(new Goomba(71*t, (groundRow-1)*t));
    enemies.push(new PiranhaPlant(20*t, (groundRow-2)*t));
    // Gap
    for(let gx=75; gx<80; gx++) {
      for(let gy=groundRow; gy<=groundRow+1; gy++) {
        const idx=tiles.findIndex(ti=>ti.gx===gx&&ti.gy===gy);
...
function checkFlagpole(mario, level) {
  ...
  if (!flag) return;
  const t = TILE();
  const fx = flag.gx * t;  // world-space
...
function update() {
  ...
  // Bowser fireballs — world-space
  for (const fb of bowserFireballs) {
    fb.x += fb.vx; fb.y += fb.vy;
    fb.vy += 0.2;
    if (fb.x < -TILE() * 2 || fb.x > level.width + TILE() * 2 || fb.y > canvas.height) fb.alive = false;
...
function render() {
  ...
  for (const fb of bowserFireballs) {
    drawPixelSprite(FIREBALL, Math.round(fb.x - game.cameraX), Math.round(fb.y));
  }
  // Draw Mario
...
function init() {
  updateHUD();
  // Detect touch device for control hint
...
// Prevent body scroll / bounce on mobile (important for APK WebView)
document.body.addEventListener('touchmove', e => e.preventDefault(), { passive: false });
document.body.addEventListener('touchstart', e => {
  // Allow touch on game controls, block elsewhere to prevent accidental scrolling
  if (!e.target.closest('#mobile-controls') && !e.target.closest('#overlay-btn')) {
    e.preventDefault();
  }
}, { passive: false });
